...
In your first session, agnes prompts you to [set up an LLM (Large Language Model) provider](/docs/getting-started/installation#set-llm-provider).
:::

<Tabs groupId="interface">
    <TabItem value="ui" label="agnes Desktop" default>
        When you open agnes, you'll see the session interface ready for use. Just type&mdash;[or speak](/docs/guides/sessions/in-session-actions#voice-dictation "Learn how to enable voice dictation")&mdash;your questions, requests, or instructions directly into the input field, and agnes will immediately get to work.
        ...
        On macOS, you can also use the agnes dock icon to quickly start sessions:
            - **Drag and drop** a folder onto the agnes icon to open a new session in that directory
            - **Right-click** the agnes icon and select `New Window` to open a new session in your most recent directory
        :::
        
        #### Keyboard Shortcuts
        
        You can also use keyboard shortcuts to start a new session or manage agnes windows.
        ...
        | Focus agnes Window | `Cmd+Option+G` | `Ctrl+Alt+G` |
        | Keep agnes Window Always on Top | `Cmd+Shift+T` | `Ctrl+Shift+T` |
        ...
        Changes to global shortcuts (Focus Window, Quick Launcher) take effect immediately. Changes to application shortcuts (New Chat, Settings, etc.) require restarting agnes.
        ...
        The session opens to your most recently opened directory in a new agnes window.

    </TabItem>
    <TabItem value="cli" label="agnes CLI">
        From your terminal, navigate to the directory from which you'd like to start, and run the [session](/docs/guides/agnes-cli-commands#session-options) command:
        ```sh
        agnes session 
        ```
...
<Tabs groupId="interface">
    <TabItem value="ui" label="agnes Desktop" default>
        A session's display name is auto-generated based on the context of your initial prompt. Session names help you identify agnes sessions so you can switch between active sessions or [resume a session](#resume-session).
    ...
    <TabItem value="cli" label="agnes CLI">
        agnes sessions are given an auto-generated name based on the context of your initial prompt.
        ...
        ```sh
        agnes session --name react-migration
        ```

        If you want to confirm the session name, run:

        ```sh
        agnes session list -l 1
        ```
        ...
        In the output above, `20260213_9` is the session ID. Session IDs use the format `YYYYMMDD_<COUNT>`. Many [agnes CLI commands](/docs/guides/agnes-cli-commands) let you identify a session by name (`--name` / `-n`) as an alternative to `--session-id`.
    </TabItem>
</Tabs>

:::tip Disable AI-generated Session Naming
Use [`AGNES_DISABLE_SESSION_NAMING`](/docs/guides/environment-variables#session-management) to keep the default name instead of calling a model to generate one (useful in CI/headless workflows). The default name in agnes Desktop is "New Chat" and the default name in agnes CLI is "CLI Session".
:::
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="agnes CLI">
        ...
        Your session will be stored in agnes's [local SQLite database](/docs/guides/logs#session-records).
    </TabItem>
</Tabs>
...
<Tabs groupId="interface">
  <TabItem value="ui" label="agnes Desktop" default>
    ...
    You can use keyboard shortcuts and search bar buttons to search sessions in agnes Desktop.
  ...
  <TabItem value="cli" label="agnes CLI">
    ...
    To search conversation content across all your sessions, start a agnes session and ask directly:

    - "Find my earlier conversation about React hooks from last week"
    - "Show me sessions where I worked on database migrations"

    agnes will search your session history and show relevant conversations with context from matching sessions.
    ...
    The [`session list`](/docs/guides/agnes-cli-commands#session-list-options) subcommand with supported options can be useful for some search operations.

    You can also query the SQLite database directly:

    ```bash
    # Search session descriptions
    sqlite3 ~/.local/share/agnes/sessions/sessions.db \
    ...
    Starting with version 1.10.0, agnes uses a SQLite database (`sessions.db`) instead of individual `.jsonl` files. Legacy `.jsonl` files remain on disk but are no longer managed by agnes.
    :::
...
<Tabs groupId="interface">
    ...
    agnes Desktop allows you to switch between multiple chat sessions in the same window. You can start a task in one session, switch to another to do some work, and return to the first session when the task is completed. 
    ...
    <TabItem value="cli" label="agnes CLI">
        ...
        ```
        agnes session -r --name <name>
        ```
        For example, to resume the session named `react-migration`, you would run:

        ```
        agnes session -r --name react-migration
        ```
    </TabItem>
</Tabs>

Sessions created in agnes Desktop can be resumed in the CLI and vice versa. All sessions are stored in the [same database](/docs/guides/logs#session-records).
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="agnes CLI">
        You can use the [`project`](/docs/guides/agnes-cli-commands#project) and [`projects`](/docs/guides/agnes-cli-commands#projects) commands to start or resume sessions from a project, which is a tracked working directory with session metadata. For a complete guide to using Projects, see [Managing Projects Guide](/docs/guides/managing-projects).
    </TabItem>
</Tabs>
...
<Tabs groupId="interface">
    <TabItem value="ui" label="agnes Desktop" default>
        ...
        Deleting a session from agnes Desktop will also delete it from the CLI. This action cannot be undone.
        :::
    ...
    <TabItem value="cli" label="agnes CLI">
        You can remove sessions using CLI commands. For detailed instructions on session removal, see the [CLI Commands documentation](/docs/guides/agnes-cli-commands#session-remove-options).
    </TabItem>
</Tabs>
...
<Tabs groupId="interface">
    <TabItem value="ui" label="agnes Desktop" default>
        Import complete sessions from JSON files to restore, share, or migrate sessions between agnes instances. Importing creates a new session with a new ID rather than overwriting existing sessions.

        1. Click <History className="inline" size={16} /> `Session History` in the sidebar
        2. Click the <Upload className="inline" size={16} /> `Import Session` button in the top-right corner
        3. Select a `.json` session file that was previously exported from agnes
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="agnes CLI">
        ...
        From your terminal, run the [`session export`](/docs/guides/agnes-cli-commands#session-export-options) subcommand:
