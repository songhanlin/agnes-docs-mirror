...
|---------------------|----------------------------------------|---------------------------------------------|
| **Command History** | `~/.config/agnes/history.txt`          | `%APPDATA%\Block\agnes\data\history.txt`    |
...
* Unix-like: ` ~/.config/agnes/history.txt`
...
Session IDs are named using `YYYYMMDD_<COUNT>` format, for example: `20250310_2`. agnes CLI outputs the session ID at the start of each session. To get session IDs, use [`agnes session list` command](/docs/guides/agnes-cli-commands#session-list-options) to see all available sessions.
...
The Server logs contain information about the agnes daemon (`agnesd`), which is a local server process that runs on your computer. This server component manages communication between the CLI, extensions, and LLMs. 
