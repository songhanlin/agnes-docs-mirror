...
`.gooseignore` is a text file that defines patterns for files and directories that Agnes will not access. This means Agnes cannot read, modify, delete, or run shell commands on these files when using the Developer extension's tools.
...
This guide will show you how to use `.gooseignore` files to prevent Agnes from changing specific files and directories.

## Creating your `.gooseignore` file

Agnes supports two types of `.gooseignore` files:
- **Global ignore file** - Create a `.gooseignore` file in `~/.agnes`. These restrictions will apply to all your sessions with Agnes, regardless of directory.
...
:::tip
You can use both global and local `.gooseignore` files simultaneously. When both exist, Agnes will apply patterns from both files, with local patterns able to override global ones using negation.
:::

## Example `.gooseignore` file

In your `.gooseignore` file, you can write patterns to match files you want Agnes to ignore. Here are some common patterns:
...
Agnes respects ignore rules from global and local `.gooseignore` files, using a priority system where later patterns can override earlier ones.
...
If you haven't created any `.gooseignore` files (neither global nor local), Agnes automatically protects these sensitive files:
...
- **Custom Restrictions**: Create `.gooseignore` files to define which files Agnes should not access 
