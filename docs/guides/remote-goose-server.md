...
Agnes Desktop normally runs its own `goosed` server process in the background on the same machine. You can also run `goosed` separately — for example, on a remote VM or a different machine on your network — and point Agnes Desktop at it.
...
Agnes Desktop will refuse to connect to a remote `goosed` server over plain HTTP. TLS is enabled by default (`AGNES_TLS=true`), so make sure you have not disabled it.
:::
...
/Applications/Agnes.app/Contents/Resources/bin/goosed agent
```

On Linux or Windows the path to the `goosed` binary will differ — use the one bundled with your Agnes installation, or a standalone `goosed` build.
...
Because `goosed` generates a self-signed TLS certificate, Agnes Desktop pins it by SHA-256 fingerprint rather than relying on a public certificate authority.
...
:::note
The fingerprint changes whenever `goosed` regenerates its certificate (for example, if you delete the cert file). If Agnes Desktop suddenly refuses to connect after a server restart, re-check the fingerprint.
:::
...
After saving, Agnes Desktop will route all backend requests to the remote `goosed`. If the connection fails, see [Troubleshooting](#troubleshooting).
...
Create a LaunchAgent plist at `~/Library/LaunchAgents/com.agnes.goosed.external.plist`:
...
<plist version="1.0">
  <dict>
    <key>Label</key>
    <string>com.agnes.goosed.external</string>

    <key>ProgramArguments</key>
    <array>
      <string>/Applications/Agnes.app/Contents/Resources/bin/goosed</string>
      <string>agent</string>
    </array>
...
```bash
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.agnes.goosed.external.plist
launchctl kickstart -k gui/$(id -u)/com.agnes.goosed.external
```

To stop or remove it later:

```bash
launchctl bootout gui/$(id -u)/com.agnes.goosed.external
```

:::tip
Because the secret key is stored in plain text in the plist, the file should be readable only by your user. macOS LaunchAgents under `~/Library/LaunchAgents/` are already user-scoped, but you can tighten further with `chmod 600 ~/Library/LaunchAgents/com.agnes.goosed.external.plist`.
:::
...
If `curl` works from the server but the client machine times out or gets "connection refused", check what interface `goosed` is bound to. If `AGNES_HOST` is `localhost` or `127.0.0.1`, only loopback connections are accepted.

Set `AGNES_HOST=0.0.0.0` to accept connections on all interfaces, then restart `goosed`. You can verify with:
...
- If you see `listening on http://...`, TLS is **not** enabled. Agnes Desktop will not connect. Set `AGNES_TLS=true` and restart `goosed`.
- If you see `listening on https://...`, TLS is enabled and you are good to go.

The startup logs also contain the `GOOSED_CERT_FINGERPRINT=...` line you need for the Agnes Desktop configuration. Search the server's stdout (or log file, if running under `launchd`) for `GOOSED_CERT_FINGERPRINT` to find it.
...
- The server regenerated its certificate (for example, after deleting the cert file). Look at the latest startup logs for the current `GOOSED_CERT_FINGERPRINT` and update Agnes Desktop.
